...
- 缺岗补位：从日期和班次发起缺岗登记，按同科室、同岗位、技能标签匹配当天未排班或休息的候选人；选定后校验夜班后接白班、连续工作上限和同日重复排班，不合规则拒绝并说明原因；通过后替补人员班次与缺岗状态同步更新，审计留痕发起人、原安排、补位人和处理时间，已处理的缺岗不能重复补位。
...
- 前端 Nginx 将 `/api/` 请求反向代理到 `http://backend:8080`（保留 `/api` 前缀），前端代码不硬编码 `localhost`。
